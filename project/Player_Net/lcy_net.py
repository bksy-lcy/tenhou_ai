...
class PolicyValueNet():
    ...
    def __init__(self, model_file=None, use_gpu=True):
        ...
        if self.use_gpu:
            ...
            self.pong_net = Net(num_output=4).cuda()
            self.chow_net = Net(num_output=4).cuda()
        else:
            self.draw_net = Net()
            self.dora_net = Net()
            ...
            self.pong_net = Net(num_output=4)
            self.chow_net = Net(num_output=4)
        ...
        self.action_types=['draw', 'dora', 'discard', 'ron', 'kong', 'pong', 'chow']
        if model_file:
            self.load_model()
